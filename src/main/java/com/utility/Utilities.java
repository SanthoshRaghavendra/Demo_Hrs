...
//			logger.error(e);
     ...
     * @return true, if successful
     */
    public static boolean verifyElementPresentAndText(By byLocator, String validationtext) throws Exception {
    	  String actualText= getText(byLocator);
        try {
            WebElement element = findElement(byLocator);
         
            softAssert.assertEquals(actualText, validationtext, " " + validationtext + " " + " is displayed");
            logger.info(validationtext + " is displayed");
            ExtentReporter.extentLoggerPass("checkElementPresent", "" + validationtext + " is displayed");
            return true;
        } catch (Exception e) {
            softAssert.assertEquals(actualText, validationtext,  validationtext + " " + " is not displayed");
...
//====================================================================================================================================
	 ...
	 * @throws Exception
	 */
	public static LinkedList<String> getAllvaluesFromDD(By element) throws Exception {
...
//			logger.error(e);
	...
	public static void switchPlatformToWebToAPI(String url) {
		new DriverInstance().LaunchBrowser("Chrome");
		waitTime(5000);
		getWebDriver().get(url);
	}
...
//		System.out.println(getPlatform());
	...
	public static void findElementAndTypeWeb(By byLocator, String value, String FieldName) throws Exception {
		waitTime(2000);
		WebElement ele = getWebDriver().findElement(byLocator);
		if(ele.isDisplayed())
		{
			getWebDriver().findElement(byLocator).sendKeys(value);
			logger.info("Typed the value " + value + " into " + FieldName);
			ExtentReporter.extentLoggerPass("", "Typed the value " + value + " into " + FieldName);
		}
		else {
			
			logger.info("Not Typed the value " + value + " into " + FieldName);
			ExtentReporter.extentLoggerFail("", "Not Typed the value " + value + " into " + FieldName);
		}
		
	}

	public static void findElementAndClickWeb(By byLocator, String FieldName) {
		waitTime(2000);
		getWebDriver().findElement(byLocator).click();
		logger.info("Clicked element " + FieldName);
		ExtentReporter.extentLogger("clickedElement", "Clicked element " + FieldName);
	}
	...
	public static void findElementAndVerifyTextWeb(By byLocator, String expectedtext)throws Exception {
		...
		try {
			element = getWebDriver().findElement(byLocator);
			String nameWithProperSpacing = element.getText().replaceAll("\\s+", " ");
			explicitWaitVisibility(byLocator, 40);
			if (element.isDisplayed()) {
				softAssert.assertEquals(nameWithProperSpacing, expectedtext);
				logger.info(nameWithProperSpacing + " is displayed and matched");
				ExtentReporter.extentLoggerPass("Pass",nameWithProperSpacing+ " is displayed and matched");
			...
			//	softAssert.assertEquals(nameWithProperSpacing, expectedtext);
				logger.error(expectedtext +" is not displayed and matched");
				ExtentReporter.extentLoggerFail("Fail", expectedtext + " is not displayed and matched");
			}
		} catch (Exception e) {
			logger.error(e.getMessage());
			ExtentReporter.extentLoggerFail("testcase", e.getMessage());
...
//		int elementWid=elementSize.getWidth();
	...
	public static void validateColorCode(By element )throws Exception {
		  try {
			  WebElement ele = getWebDriver().findElement(element);
			  String actualColorCode="";
		   Wait(200);
		   if ((ele.getCssValue("background-color")).equalsIgnoreCase("rgba(255, 255, 255, 1)")) {
				  actualColorCode = ele.getCssValue("background-color");
		    softAssert.assertEquals(actualColorCode, "rgba(255, 255, 255, 1)");  
		   
		    logger.info(ele.getText()+ "   Button color is  " + "  White");
		    ExtentReporter.extentLoggerPass("Pass", ele.getText()+ "   Button color is  " + "  White" );
		   } 
		   else if((ele.getCssValue("background-color")).equalsIgnoreCase("rgba(243, 152, 52, 1)")) {
			   actualColorCode = ele.getCssValue("background-color");
			    softAssert.assertEquals(actualColorCode, "rgba(243, 152, 52, 1)");
			    logger.info(ele.getText()+ "   Button color is  " + "  Orange");
			    ExtentReporter.extentLoggerPass("Pass", ele.getText()+ "   Button color is  " + "  Orange" );
		   }
		   else {
				 logger.error( ele.getText()+ " Button color is "+  " not matched");
			    ExtentReporter.extentLoggerFail("Fail",  ele.getText()+ " Button color is "+  " not matched");
		   }
		  }  catch (Exception e) {
		   logger.error(e.getMessage());
		   ExtentReporter.extentLoggerFail("testcase", e.getMessage());
	...
	public static void validateColorCodeByWebElement(WebElement element )throws Exception {
		  try {
			  String actualColorCode="";
		   Wait(200);
		   if ((element.getCssValue("background-color")).equalsIgnoreCase("rgba(255, 255, 255, 1)")) {
				  actualColorCode = element.getCssValue("background-color");
		    softAssert.assertEquals(actualColorCode, "rgba(255, 255, 255, 1)");  
		   
		    logger.info(element.getText()+ "   Button color is  " + "  White");
		    ExtentReporter.extentLoggerPass("Pass", element.getText()+ "   Button color is  " + "  White" );
		   } 
		   else if((element.getCssValue("background-color")).equalsIgnoreCase("rgba(243, 152, 52, 1)")) {
			   actualColorCode = element.getCssValue("background-color");
			    softAssert.assertEquals(actualColorCode, "rgba(243, 152, 52, 1)");
			    logger.info(element.getText()+ "   Button color is  " + "  Orange");
			    ExtentReporter.extentLoggerPass("Pass", element.getText()+ "   Button color is  " + "  Orange" );
		   }
		   else {
				 logger.error( element.getText()+ " Button color is "+  " not matched");
			    ExtentReporter.extentLoggerFail("Fail",  element.getText()+ " Button color is "+  " not matched");
		   }
		  } catch (Exception e) {
		   logger.error(e.getMessage());
		   ExtentReporter.extentLoggerFail("testcase", e.getMessage());
     ...
     */
    @SuppressWarnings("rawtypes")
    public static void SwipeWeb(String direction, int count) {
    ...
    public static void scrollTableHorizontally( WebElement table, int scrollDistance) {
        JavascriptExecutor js = (JavascriptExecutor) getWebDriver();

        // Scroll the table horizontally using JavaScript
        js.executeScript("arguments[0].scrollLeft += arguments[1];", table, scrollDistance);
    }
	
    
    public static void midScreenClick() {
    	Dimension window = getWebDriver().manage().window().getSize();
    ...
    public static void slideByPercentage(WebDriver Webdriver,  double percentage) throws Exception {
        try {
        	WebElement ele = getWebDriver().findElement(By.xpath("(//*[@class='ui-grid-viewport ng-isolate-scope' ])[2]"));
            Actions act = new Actions(Webdriver);
            int endingPoint = ele.getSize().getWidth();
            int startingPoint = ele.getLocation().getX();
            double range = ((endingPoint - startingPoint)) * (percentage / 100);
            int startPoint = (endingPoint - startingPoint) / 2;
            int position = (int) range;
            act.moveToElement(ele).moveToElement(ele).build().perform();
            if (verifyElementDisplayed((By.xpath("(//*[@class='ui-grid-viewport ng-isolate-scope' ])[2]")))) {

                if (percentage <= 50) {
                    act.moveToElement(ele).clickAndHold().moveByOffset(-startPoint + position, 0);
                } else {
                    act.moveToElement(ele).clickAndHold().moveByOffset((-startPoint + position) + 17, 0);
                }
                act.release().perform();
                logger.info("Is swipe");
            } else {
            	 logger.info("Is swipe");
            }
        } catch (Exception e) {
	...
	public static void verifyElementDisplayedinPage(By locator, String element) {
		try {
			  WebElement ele = getWebDriver().findElement(locator);
		        if (!ele.isDisplayed()) {
		        	logger.info(element +"   Element is not displayed on the page. ");
					ExtentReporter.extentLogger(element, "   Element is not displayed on the page. ");
		        } else { 
		        	logger.info(element +"   Element is displayed on the page. ");
					ExtentReporter.extentLogger(element, "   Element is displayed on the page. ");
		        }
		} catch (NoSuchElementException e) {
			logger.info("Did not click on " + element);
			ExtentReporter.extentLogger("Did not Click on", " " + element);
		}
	}
